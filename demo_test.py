...
def test(test_loader, cfg):
    ...
    pretrained_dict = torch.load('log/x' + str(cfg.scale_factor) + '/PASSRnet_x' + str(cfg.scale_factor) + '.pth')['state_dict']
    ...
    with torch.no_grad():
        for idx_iter, (LR_left, LR_right, HR_left, Pos) in enumerate(test_loader):
            ...
            SR_left = net(LR_left, LR_right, is_training=0, Pos=Pos)
...
#             SR_left_img.save('results/'+cfg.dataset+'/'+scene_name+'/img_0.png')
            cv2.imwrite('results/'+cfg.dataset+'/'+scene_name+'/left_sr.png', exr2uint8(SR_left))
            cv2.imwrite('results/'+cfg.dataset+'/'+scene_name+'/left_lr.png', exr2uint8(LR_left))
            cv2.imwrite('results/'+cfg.dataset+'/'+scene_name+'/right_lr.png', exr2uint8(LR_right))
            cv2.imwrite('results/'+cfg.dataset+'/'+scene_name+'/left_hr.png', exr2uint8(HR_left))

        ## print results
        print(cfg.dataset + ' mean psnr: ', float(np.array(psnr_list).mean()))

def main(cfg):
    test_set = Train_or_Evalset_DUAL(cfg, 256, False)
